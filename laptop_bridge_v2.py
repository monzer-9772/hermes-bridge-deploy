...
@app.get("/queue/worker_tick", response_model=WorkerTickRes, dependencies=[Depends(verify_token)])
...
def background_worker():
    """Background thread that auto-ticks the queue every 2 seconds."""
    import httpx as hx
    client = hx.Client(timeout=30, headers={"Authorization": f"Bearer {BRIDGE_TOKEN}"})
    while not _worker_stop.is_set():
        try:
            r = client.post(f"http://127.0.0.1:8765/queue/worker_tick")
